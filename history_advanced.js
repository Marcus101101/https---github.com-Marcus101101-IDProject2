...
$(document).ready(function () {
    ...
    function getContacts(limit = 100, all = true) {
      ...
      let settings = {
        "async": true,
        "crossDomain": true,
        "url": "https://idleaderboardfinal-5736.restdb.io/rest/history-advanced",
        ...
        for (var i = 0; i < response.length && i < limit; i++) {
          ...
          for (var i = 0; i < response.length && i < limit; i++) {
            ...
            for (var r = 0; r < 1; r++) {
              ...
              content = `${content}<tr id='${response[i]._id}'><td>${rank}</td><td>${response[i].name}</td><td>${response[i].score}</td></tr>`;
              rank++;
            }
    ...
    function updateForm(id, contactName, contactMsg) {
      ...
      var settings = {
        "async": true,
        "crossDomain": true,
        "url": `https://idleaderboardfinal-5736.restdb.io/rest/history-advanced${id}`,//update based on the ID
        "method": "PUT",
        "headers": {
          "content-type": "application/json",
          "x-apikey": APIKEY,
